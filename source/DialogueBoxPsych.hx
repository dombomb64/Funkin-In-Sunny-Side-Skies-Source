...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	public var offsetPos:Float = -600;
	...
	override function update(elapsed:Float)
	{
		...
		if(!dialogueEnded) {
			...
			if(PlayerSettings.player1.controls.ACCEPT) {
				if(!daText.finishedText && !FlxG.keys.pressed.SHIFT) {
					daText.finishText();
					if(skipDialogueThing != null) {
						skipDialogueThing();
					}
				} else if(currentText >= dialogueList.dialogue.length || FlxG.keys.anyPressed([FlxKey.SHIFT])) {
					if (FlxG.keys.pressed.SHIFT) { // Dialogue skipped
						FlxG.sound.play(Paths.sound('scrollMenu' + ClientPrefs.menuSoundSuffix));

						var sonicsDeath = new Alphabet(0, 0, 'Dialogue Skipped');
						sonicsDeath.screenCenter();
						add(sonicsDeath);

						for (letter in sonicsDeath.letters) {
							letter.acceleration.y = FlxG.random.int(200, 300) * 2;
							letter.velocity.y -= FlxG.random.int(140, 160) * 2;
							letter.velocity.x = FlxG.random.float(-5, 5) * 2;

							FlxTween.tween(letter, {alpha: 0}, 0.2, {
			...
			if (!(MusicBeatState.getState() is FinaleDialogueState))
			{
				for (i in 0...arrayCharacters.length) {
	...
	public static function updateBoxOffsets(box:FlxSprite) { //Had to make it static because of the editors
		box.centerOffsets();
		box.updateHitbox();
		if(box.animation.curAnim != null && box.animation.curAnim.name.startsWith('angry')) {
			box.offset.set(50, 65);
		} else if(box.animation.curAnim != null && box.animation.curAnim.name.startsWith('center-angry')) {
