...
function onStartCountdown()
	--luaDebugMode = true;
	--debugPrint(tostring(dialogueFinished) .. ', ' .. tostring(isStoryMode) .. ', ' .. tostring(seenCutscene));
	--debugPrint(getPropertyFromGroup('luaArray', 1, 'scriptName'));
	if not cutsceneFinished and isStoryMode and not seenCutscene then
		--debugPrint('cutscene started');
		startVideo('bestFriend');
...
function onTimerCompleted(tag, loops, loopsLeft)
	if tag == 'startDialogue' then -- Timer completed, play dialogue
		...
		--debugPrint('dialogue actually started');
	end
end
