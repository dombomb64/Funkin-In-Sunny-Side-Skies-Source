...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		blackBG = new FlxSprite(posX - 25).makeGraphic(870, 200, FlxColor.WHITE); // Ha. Ironic.
		blackBG.alpha = 0.4;
		add(blackBG);
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			for (j in 0...3) {
				var optionText:Alphabet = new Alphabet(posX + (225 * j) + 250, yPos + 60, Std.string(ClientPrefs.arrowHSV[i][j]), false, 'alphabetWhite');
				optionText.y -= 55;
		...
		hsbText = new Alphabet(posX + 560, 0, "Hue    Saturation  Brightness", false, 'alphabetWhite');
		hsbText.scaleX = 0.6;
		hsbText.scaleY = 0.6;
		hsbText.y -= 22.5;
		add(hsbText);
