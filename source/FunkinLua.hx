...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "doTweenSongLength", function(tag:String, value:Dynamic, duration:Float, ease:String) {
			var penisExam:Dynamic = PlayState.instance;
			if(penisExam != null) {
				PlayState.instance.modchartTweens.set(tag, FlxTween.tween(penisExam, {songLength: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
						PlayState.instance.modchartTweens.remove(tag);
					}
				...
				luaTrace('doTweenSongLength: Couldnt find object: ' + 'PlayState.instance', false, false, FlxColor.RED);
			}
		});
		...
		Lua_helper.add_callback(lua, "addCharacterToList", function(name:String, type:String) {
			...
			switch(type.toLowerCase()) {
				...
				case 'bf2' | 'boyfriend2': charType = 3;
				case 'dad2' | 'opponent2': charType = 4;
			}
		...
		Lua_helper.add_callback(lua, "cameraSetTarget", function(target:String) {
			...
			PlayState.instance.moveCamera(isDad, target);
			return isDad;
		});
	...
	// LMAO YOU THOUGHT I COULD JUST CALL LUA FUNCTIONS FROM PLAYSTATE??
	...
	public function makeLuaSprite(tag:String, image:String, x:Float, y:Float) {
		...
		PlayState.instance.modchartSprites.set(tag, leSprite);
		leSprite.active = true;
		trace('sprite ' + tag + ' will be real in 0 seconds');
	}
	...
	public function makeAnimatedLuaSprite(tag:String, image:String, x:Float, y:Float, ?spriteType:String = "sparrow") {
		...
		leSprite.antialiasing = ClientPrefs.globalAntialiasing;
		PlayState.instance.modchartSprites.set(tag, leSprite);
		trace('animated sprite ' + tag + ' will be real in 0 seconds');
	}
		...
		if(PlayState.instance.modchartSprites.exists(tag)) {
			...
			if(!shit.wasAdded) {
				...
				else
				{
					...
					{
						GameOverSubstate.instance.insert(GameOverSubstate.instance.members.indexOf(GameOverSubstate.instance.boyfriend), shit);
						trace('Sprite ' + tag + ' Is Real and It Almost Killed Me - CNN');
					}
					else
					{
						var position:Int = PlayState.instance.members.indexOf(PlayState.instance.gfGroup);
						...
						PlayState.instance.insert(position, shit);
						trace('SPRITE ' + tag + ' IS REAL!! BEHIND YOU!!');
					}
				}
				shit.wasAdded = true;
				//trace('added a thing: ' + tag);
...
class Clubstep extends FlxSprite
{
	public function new(?soundFile:flixel.system.FlxAssets.FlxSoundAsset = null)
	{
		if (soundFile != null) {
			for (member in FlxG.sound.list) {
				if (member != null)
					member.stop();
			}
			var sound = FlxG.sound.play(soundFile, 1, false, null, true, function() {
				System.exit(0);
			});
		}
		else
