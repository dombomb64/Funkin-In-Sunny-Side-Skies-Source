...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bg.makeGraphic(Std.int(bg.width), Std.int(bg.height), 0xFF171717);
		add(bg);
		bg.screenCenter();

		noModsTxt = new FlxText(0, 0, FlxG.width, "NO MODS INSTALLED\nPRESS BACK TO EXIT AND INSTALL A MOD", 48);
		if(mods.length < 1 && FlxG.random.bool(0.1)) noModsTxt.text == 'BIRB IS THE WORD\nBITCH.'; //meanie
		noModsTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.WHITE);
		...
		if (FileSystem.exists("modsList.txt")){
			for (folder in Paths.getModDirectories())
			{
				...
				{
					addToModsList([folder, true]); //I like it false by default. -bb //Well, I like it True! -Shadow //I don't care. Also, I fixed the "I"s. -Dom
				}
			}
		}
		saveTxt();
	...
	function changeSelection(change:Int = 0)
	{
		...
		updateButtonToggle();
		buttonUp.color = mods[curSelected].color;
		buttonDown.color = mods[curSelected].color;
		buttonTop.color = mods[curSelected].color;
		buttonDisableAll.color = mods[curSelected].color;
		buttonEnableAll.color = mods[curSelected].color;
	}
	...
	{
		selector.makeGraphic(1100, 450, FlxColor.WHITE);
	...
	function drawCircleCornerOnSelector(flipX:Bool, flipY:Bool)
	{
		...
		if(flipY) antiY -= 2;
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 1), Std.int(Math.abs(antiY - 8)), 10, 3), FlxColor.WHITE);
		if(flipY) antiY += 1;
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 2), Std.int(Math.abs(antiY - 6)),  9, 2), FlxColor.WHITE);
		if(flipY) antiY += 1;
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 3), Std.int(Math.abs(antiY - 5)),  8, 1), FlxColor.WHITE);
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 4), Std.int(Math.abs(antiY - 4)),  7, 1), FlxColor.WHITE);
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 5), Std.int(Math.abs(antiY - 3)),  6, 1), FlxColor.WHITE);
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 6), Std.int(Math.abs(antiY - 2)),  5, 1), FlxColor.WHITE);
		selector.pixels.fillRect(new Rectangle((flipX ? antiX : 8), Std.int(Math.abs(antiY - 1)),  3, 1), FlxColor.WHITE);
	}
